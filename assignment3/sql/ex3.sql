...
/* Insert Favourite parts for a specific user (with userId 164) that the user has used in any of their builds and aren't already in their favourites */
INSERT INTO UserFavouritePart 
SELECT DISTINCT 164, p.partNo
FROM Part as p
WHERE (EXISTS
	(SELECT bp.partNo, b.userId FROM BuildPart as bp, Build as b
	WHERE bp.buildNo = b.buildNo AND b.userId = 164 AND p.partNo = bp.partNo)
